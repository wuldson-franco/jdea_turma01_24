...
def google_sheet_to_minio_etl(sheet_id, sheet_name, bucket_name, endpoint_url, access_key, secret_key):
    ...
    try:
        ...
        minio_client.put_object(Bucket=bucket_name, Key=f"bk_ronaldinho/landing/{sheet_name}/lnd_{sheet_name}.parquet", Body=parquet_buffer.getvalue())
    except Exception as e:
        logging.error(f"Erro ao processar a planilha lnd_{sheet_name}: {e}")
        raise
    ...
    try:
        with connection.cursor() as cursor:
            ...
            create_table_sql = f"""
            CREATE TABLE IF NOT EXISTS lnd_{sheet_name} (
                {', '.join([f'{col} VARCHAR(255)' for col in df.columns])}
            )
            """
            cursor.execute(create_table_sql)
            logging.info(f"Tabela lnd_{sheet_name} verificada/criada no MariaDB")

            # Inserir ou atualizar dados na tabela
            for index, row in df.iterrows():
                # Atualizar dados se já existir
                update_sql = f"""
                UPDATE lnd_{sheet_name}
                ...
                insert_sql = f"""
                INSERT IGNORE INTO lnd_{sheet_name} ({', '.join(df.columns)})
                VALUES ({', '.join(['%s'] * len(df.columns))})
                """
                cursor.execute(insert_sql, tuple(row))

            connection.commit()
            logging.info(f"Dados inseridos/atualizados na tabela lnd_{sheet_name} no MariaDB")
