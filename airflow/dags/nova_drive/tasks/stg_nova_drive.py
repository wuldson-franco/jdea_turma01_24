...
def postgres_to_minio_etl_parquet(table_name: str, bucket_name: str, endpoint_url: str, access_key: str, secret_key: str):
    ...
    try:
        obj = s3_client.get_object(Bucket=bucket_name, Key=f"nova_drive/staging/{table_name}/max_id.txt")
    ...
    try:
        with pg_conn.cursor() as pg_cursor:
            ...
            pg_cursor.execute(f"SELECT column_name FROM information_schema.columns WHERE table_name = 'stg_{table_name}'")
            columns = [row[0] for row in pg_cursor.fetchall()]
            columns_list_str = ', '.join(columns)

            pg_cursor.execute(f"SELECT {columns_list_str} FROM stg_{table_name} WHERE {primary_key} > %s", (max_id,))
            ...
            if rows:
                ...
                s3_client.put_object(Bucket=bucket_name, Key=f"nova_drive/staging/{table_name}/data_{max_id + 1}.parquet", Body=parquet_buffer.getvalue())
                max_id = df[primary_key].max()
                s3_client.put_object(Bucket=bucket_name, Key=f"nova_drive/staging/{table_name}/max_id.txt", Body=str(max_id))
    ...
    if df is not None:
        ...
        try:
            with connection.cursor() as cursor:
                ...
                logger.info(f"Tabela stg_{table_name} verificada/criada no MariaDB")

                # Inserir ou atualizar dados na tabela
                for index, row in df.iterrows():
                    # Atualizar dados se já existir
                    update_sql = f"""
                    UPDATE stg_{table_name}
                    ...
                    insert_sql = f"""
                    INSERT IGNORE INTO stg_{table_name} ({', '.join(columns)})
                    VALUES ({', '.join(['%s'] * len(columns))})
                    """
                    cursor.execute(insert_sql, tuple(row))

                connection.commit()
                logger.info(f"Dados inseridos/atualizados na tabela stg_{table_name} no MariaDB")
