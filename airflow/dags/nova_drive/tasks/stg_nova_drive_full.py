...
def postgres_to_minio_etl_parquet_full(table_name: str, bucket_name: str, endpoint_url: str, access_key: str, secret_key: str):
    ...
    try:
        with pg_conn.cursor() as pg_cursor:
            ...
            if rows:
                ...
                s3_client.put_object(Bucket=bucket_name, Key=f"nova_drive/staging/{table_name}/stg_{table_name}.parquet", Body=parquet_buffer.getvalue())
    ...
    try:
        with connection.cursor() as cursor:
            ...
            cursor.execute(f"TRUNCATE TABLE stg_{table_name}")
            logger.info(f"Tabela stg_{table_name} limpa antes da carga")

            # Inserir dados na tabela
            for index, row in df.iterrows():
                insert_sql = f"""
                INSERT INTO stg_{table_name} ({', '.join(columns)})
                VALUES ({', '.join(['%s'] * len(columns))})
                """
                cursor.execute(insert_sql, tuple(row))

            connection.commit()
            logger.info(f"Dados completos inseridos na tabela stg_{table_name} no MariaDB")
